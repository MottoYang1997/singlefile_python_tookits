#!/usr/bin/env python3
"""
Open-Short Deembedder (scikit-rf)

Workflow
1) Pick an OPEN Touchstone file (.s2p / .sNp)
2) Pick a SHORT Touchstone file
3) Pick one or more DUT Touchstone files
4) Choose S-parameters to plot (S11, S12, S21, S22)
5) The app de-embeds each DUT using Open-Short and plots |S| (dB) vs linear frequency
...
Notes
- This tool targets 2-port fixtures/DUTs. If files are not 2-port they are skipped with a warning.
- Open/Short are interpolated to each DUT's frequency grid prior to de-embedding.
"""
...
def interpolate_like(ref: rf.Network, target: rf.Network) -> rf.Network:
    ...
    # If identical frequency vectors, return as-is
    ...
    try:
        ...
        # Fallback to simple linear interpolation by frequency in Hz
...
def plot_deembedded(dut_pairs: List[Tuple[rf.Network, rf.Network]], params: List[str]):
    ...
    # Aggregate frequencies for unit selection
    ...
    plt.ylabel("|S| (dB)")
    plt.title("Open-Short De-embedded |S| vs Frequency")
...
def main():
    ...
    # 5) De-embed each DUT
    dut_pairs: List[Tuple[rf.Network, rf.Network]] = []  # (raw, deembedded)
    for dut in duts:
        try:
            # Interpolate standards to DUT grid
            o = interpolate_like(open_net, dut)
            s = interpolate_like(short_net, dut)

            os_deemb = OpenShort(o, s)
            de = os_deemb.deembed(dut)
            # Keep a useful name
